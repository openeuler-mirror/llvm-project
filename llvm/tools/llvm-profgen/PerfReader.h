...
enum PerfFormat {
  ...
  SPEPerfData = 4,         // Raw linux perf.data which includes SPE sample.
};
...
struct PerfSample {
  ...
  // SPE stack recordef in FIFO order.
  SmallVector<uint64_t, 16> SPEStack;
  ...
  bool isEqual(const PerfSample *Other) const {
    ...
    const SmallVector<uint64_t, 16> &OtherSPEStack = Other->SPEStack;

    if (CallStack.size() != OtherCallStack.size() ||
        LBRStack.size() != OtherLBRStack.size() ||
        SPEStack.size() != OtherSPEStack.size())
      return false;
    ...
    if (!std::equal(SPEStack.begin(), SPEStack.end(), OtherSPEStack.begin()))
      return false;
...
struct SampleCounter {
  ...
  std::unordered_map<uint64_t, uint64_t> SPECounter;
  ...
  void recordSPECount(uint64_t Addr, uint64_t Repeat) {
    SPECounter[Addr] += Repeat;
  }
};
...
protected:
  ...
  // Check whether a given line is SPE sample
  static bool isSPESample(StringRef Line);
  ...
  void computeCounterFromSPE(const PerfSample *Sample, uint64_t Repeat);
...
class SPEPerfReader : public PerfScriptReader {
public:
  SPEPerfReader(ProfiledBinary *Binary, StringRef PerfTrace,
                std::optional<int32_t> PID)
      : PerfScriptReader(Binary, PerfTrace, PID) {};
  // Parse the SPE sample.
  void parseSample(TraceStream &TraceIt, uint64_t Count) override;
};
