#ifndef LLVM_ADT_SADIAGONALWINDOWS_H
#define LLVM_ADT_SADIAGONALWINDOWS_H
...
public:
  DiagonalWindowsSA(ScoringSystem Scoring, MatchFnTy Match, size_t WindowSize)
      : BaseType(Scoring, Match), WindowSize(WindowSize) {}

  virtual size_t getMemoryRequirement(ContainerType &Seq1,
                                      ContainerType &Seq2) {
    size_t MemorySize =
        sizeof(ScoreSystemType) * (WindowSize + 1) * (WindowSize + 1);
