...
static cl::opt<bool> DisableMovStoreInsOutsideOfLoopInSigFun(
    "disable-move-store-ins-outside-of-loop", cl::Hidden, cl::init(true),
    cl::desc(
        "Disable move store instruction outside of loop in signal function."));
  ...
  for (Value *ASIV : PointerMustAliases) {
    for (Use &U : ASIV->uses()) {
      ...
      if (DisableMovStoreInsOutsideOfLoopInSigFun) {
        ...
        // loop is triggered by the signal function, the store instruction is
        // not executed.However, the function registered by the signal will read
        // the data sored in the store instruction, so the data read is
        // incorrect. Solution: Prevent the store instruction form going outside
        // the loop. NOTE: The sys_signal function takes the same arguments and
        // performs the same task as signal. They all belong to glic.
