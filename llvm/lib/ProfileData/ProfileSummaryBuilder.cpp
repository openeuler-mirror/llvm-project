...
cl::opt<int> ProfileSummaryCutoffHotICP(
    ...
    cl::desc("A count is hot in the context of ICP, if it exceeds the minimum "
             "count to"
             " reach this percentile of total counts."));
...
uint64_t ProfileSummaryBuilder::getHotCountThresholdForICP(
  ...
  uint64_t HotCountThresholdICP = HotEntry.MinCount;
  if (ProfileSummaryHotCount.getNumOccurrences() > 0)
    HotCountThresholdICP = ProfileSummaryHotCount;
  return HotCountThresholdICP;
}
