...
; RUN: llc -mattr=+sve -aarch64-movprfx-or-mov-for-unary=true -aarch64-mov-for-long-latency-inst=false < %s | FileCheck %s
; RUN: llc -mattr=+sve2p2 -aarch64-movprfx-or-mov-for-unary=true -aarch64-mov-for-long-latency-inst=false < %s | FileCheck %s -check-prefix CHECK-2p2

; RUN: llc -mattr=+sme    -force-streaming -aarch64-movprfx-or-mov-for-unary=true -aarch64-mov-for-long-latency-inst=false < %s | FileCheck %s
; RUN: llc -mattr=+sme2p2 -force-streaming -aarch64-movprfx-or-mov-for-unary=true -aarch64-mov-for-long-latency-inst=false < %s | FileCheck %s -check-prefix CHECK-2p2
