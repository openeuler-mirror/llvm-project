...
LLDB_PLUGIN_DEFINE(ABISysV_sw_64)
...
ABISysV_sw_64::GetRegisterInfoArray(uint32_t &count) {
  count = k_num_register_infos;
  return g_register_infos;
}

size_t ABISysV_sw_64::GetRedZoneSize() const { return 0; }

// Static Functions

ABISP
ABISysV_sw_64::CreateInstance(lldb::ProcessSP process_sp, const ArchSpec &arch) {
  if (arch.GetTriple().isSw64())
    return ABISP(
        new ABISysV_sw_64(std::move(process_sp), MakeMCRegisterInfo(arch)));
  return ABISP();
}

bool ABISysV_sw_64::PrepareTrivialCall(Thread &thread, addr_t sp,
  ...
  if (log) {
    StreamString s;
    s.Printf("ABISysV_sw_64::PrepareTrivialCall (tid = 0x%" PRIx64
  ...
  if (args.size() > 6) // TODO handle more than 8 arguments
    return false;
  ...
  const RegisterInfo *r25_info = reg_ctx->GetRegisterInfoByName("r27", 0);
...
bool ABISysV_sw_64::GetArgumentValues(Thread &thread,
                                       ValueList &values) const {
  return false;
}

Status ABISysV_sw_64::SetReturnValueObject(lldb::StackFrameSP &frame_sp,
...
#ifndef LHX20240722 // refer to x86_64
  bool is_signed;
  uint32_t count;
  bool is_complex;
...
ValueObjectSP ABISysV_sw_64::GetReturnValueObjectSimple(
...
#ifndef LHX20240722
ValueObjectSP ABISysV_sw_64::GetReturnValueObjectImpl(
	Thread &thread, CompilerType &return_compiler_type) const {
  ValueObjectSP return_valobj_sp;
  Value value;
  Status error;
  ...
  //uint32_t fp_flag =
  //    target_arch.GetFlags() & lldb_private::ArchSpec::eSW64_ABI_FP_mask;
  ...
  if (type_flags & eTypeIsScalar || type_flags & eTypeIsPointer) {
    ...
    if (type_flags & eTypeIsInteger || type_flags & eTypeIsPointer) {
      ...
      // In SW64 register "r0" (v0) ...
      uint64_t raw_value = reg_ctx->ReadRegisterAsUnsigned(r0_info, 0);
...
*/
        ...
        if (*byte_size <= sizeof(long double)) {
          ...
          if (*byte_size == sizeof(float)) {
            ...
            const RegisterInfo *f2_info =
                reg_ctx->GetRegisterInfoByName("f1", 0); //lhx: mips is f0,f2 sw_64 is f0,f1
    ...
    if (*byte_size <= 16) {
      ...
      if (use_r2) {
        reg_ctx->ReadRegister(r0_info, r2_value);

        const size_t bytes_copied = r2_value.GetAsMemoryData(
            *r0_info, data_sp->GetBytes(), r0_info->byte_size, target_byte_order,
            error);
        if (bytes_copied != r0_info->byte_size)
          return return_valobj_sp;
        sucess = true;
      }
#if 0
      if (use_r3) {
        ...
        const size_t bytes_copied = r3_value.GetAsMemoryData(
            r3_info, data_sp->GetBytes() + r0_info->byte_size,
...
bool ABISysV_sw_64::CreateFunctionEntryUnwindPlan(UnwindPlan &unwind_plan) {
  unwind_plan.Clear();
  ...
  row->GetCFAValue().SetIsRegisterPlusOffset(dwarf_r30, 0);

  // The previous PC is in the RA
  row->SetRegisterLocationToRegister(dwarf_pc, dwarf_r26, true);
  unwind_plan.AppendRow(row);

  // All other registers are the same.

  unwind_plan.SetSourceName("sw_64 at-func-entry default");
  unwind_plan.SetSourcedFromCompiler(eLazyBoolNo);
  unwind_plan.SetReturnAddressRegister(dwarf_r26);
  return true;
}

bool ABISysV_sw_64::CreateDefaultUnwindPlan(UnwindPlan &unwind_plan) {
  unwind_plan.Clear();
  unwind_plan.SetRegisterKind(eRegisterKindDWARF);
  ...
  unwind_plan.AppendRow(row);
  unwind_plan.SetSourceName("sw_64 default unwind plan");
  unwind_plan.SetSourcedFromCompiler(eLazyBoolNo);
...
bool ABISysV_sw_64::RegisterIsVolatile(const RegisterInfo *reg_info) {
  return !RegisterIsCalleeSaved(reg_info);
}

bool ABISysV_sw_64::IsSoftFloat(uint32_t fp_flag) const {
  //return (fp_flag == lldb_private::ArchSpec::eMIPS_ABI_FP_SOFT);
  return false;
}

bool ABISysV_sw_64::RegisterIsCalleeSaved(const RegisterInfo *reg_info) {
  if (reg_info) {
...
void ABISysV_sw_64::Initialize() {
  PluginManager::RegisterPlugin(
      GetPluginNameStatic(), "System V ABI for sw_64 targets", CreateInstance);
}

void ABISysV_sw_64::Terminate() {
  PluginManager::UnregisterPlugin(CreateInstance);
}

//lldb_private::ConstString ABISysV_sw_64::GetPluginNameStatic() {
//  static ConstString g_name("sysv-sw_64");
//  return g_name;
//}
//
//// PluginInterface protocol
//
//lldb_private::ConstString ABISysV_sw_64::GetPluginName() {
//  return GetPluginNameStatic();
//}
//
//uint32_t ABISysV_sw_64::GetPluginVersion() { return 1; }
