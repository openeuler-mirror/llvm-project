...
#include "Plugins/Architecture/Sw64/ArchitectureSw64.h"
...
LLDB_PLUGIN_DEFINE(ArchitectureSw64)

void ArchitectureSw64::Initialize() {
  PluginManager::RegisterPlugin(GetPluginNameStatic(),
                                "Sw64-specific algorithms",
                                &ArchitectureSw64::Create);
}

void ArchitectureSw64::Terminate() {
  PluginManager::UnregisterPlugin(&ArchitectureSw64::Create);
}

std::unique_ptr<Architecture> ArchitectureSw64::Create(const ArchSpec &arch) {
  return arch.IsSw64() ?
      std::unique_ptr<Architecture>(new ArchitectureSw64()) : nullptr;
}

#if 0
addr_t ArchitectureSw64::GetCallableLoadAddress(addr_t code_addr,
...
addr_t ArchitectureSw64::GetOpcodeLoadAddress(addr_t opcode_addr,
                                              AddressClass addr_class) const {
  switch (addr_class) {
  case AddressClass::eData:
  case AddressClass::eDebug:
...
lldb::addr_t ArchitectureSw64::GetBreakableLoadAddress(lldb::addr_t addr,
                                                       Target &target) const {
...
Instruction *ArchitectureSw64::GetInstructionAtAddress(
